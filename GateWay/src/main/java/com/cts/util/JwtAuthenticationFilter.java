...
public class JwtAuthenticationFilter extends AbstractGatewayFilterFactory<JwtAuthenticationFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (request.getURI().getPath().contains("/auth/login") ||
                    request.getURI().getPath().contains("/auth/register") ||
                    request.getURI().getPath().contains("/auth/validate")) {
                return chain.filter(exchange);
            }
            ...
            if (!hasRequiredRole(path, roles)) {
                return onError(exchange, "Access denied: insufficient privileges", HttpStatus.UNAUTHORIZED);
            }
    ...
    private Mono<Void> onError(ServerWebExchange exchange, String message, HttpStatus status) {
        ...
        response.getHeaders().add("Content-Type", "application/json");
        String error = "{\"error\":\"" + message + "\",\"status\":" + status.value() + "}";
